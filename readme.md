...
- compiled static files from a _bundle_ (production)
    ...
    ```
- links to files served by the Vite _development server_
    ```html
    <!-- DEVELOPMENT -->
...
To achieve this, Peat parses the _manifest_ JSON file generated by Vite for each bundle.\
...
> 💡
> 
> You may also want to set `build.outDir` to point to a subfolder in the backend's public dir,
...
In a template you would only call
```php
<head>
  <?= $vite('src/main.js') ?>
</head>
...
In Twig or Latte, for example, you may register such a function/filter to be used as follows
```twig
<head>
  {{ vite('src/main.js') }}
</head>
...
> 💡
>
> The `assetPathPrefix` parameter should contain the **project's base path** plus path to the manifest file and should be absolute.
